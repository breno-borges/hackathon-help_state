...
public class CitizenController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", content = {
                    @Content(schema = @Schema(implementation = CitizenEntity.class))
            }),
            @ApiResponse(responseCode = "400", description = "Usuario ja existe")
    })
    public ResponseEntity<Object> createCitizen(@Valid @RequestBody CitizenEntity citizenEntity) {

        try {
            CitizenEntity result = this.createCitizenUseCase.execute(citizenEntity);
